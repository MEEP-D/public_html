    ...
    <section class="section">
        ...
        {{-- <div>
            @foreach ($quizzes as $quiz)
                <h1>quiz: {{ $quiz->title }} - {{ $quiz->info }}</h1>
                @foreach ($sections->where('quiz_id', $quiz->id) as $section)
                    <h2>section: {{ $section->title }} - {{ $section->info }}</h2>
                    @foreach ($groups->where('section_id', $section->id) as $group)
                        <h2>Nhóm: {{ $group->title }} - {{ $group->info }}</h2>
                        @foreach ($questions->where('group_id', $group->id) as $question)
                            <h3>Câu hỏi {{ $question->id }}: {{ $question->content }}</h3>
                                ...
                                <div id="question-container" class="hidden text-center mt-5">
                                    ...
                                    <div class="square-container">
                                        @foreach ($questions as $question)
                                            <a href="#q{{ $question->id }}" class="square"
                                                id="square-link-{{ $question->id }}">{{ $question->id }}</a>
                                        @endforeach
                                    </div>
                                        ...
                                        <div class="card-body">
                                            <form action="/submit" method="post">
                                                {{-- @foreach ($sections as $section)
                                                    Section:{{ $section->title }} {{ $section->info }}
                                                @endforeach --}}
                                                @foreach ($groups as $group)
                                                    <div id="page{{ $loop->iteration }}"
                                                        class="page{{ $loop->first ? '' : ' d-none' }}">
                                                        <h2>Nhóm: {{ $group->title }} - {{ $group->info }}</h2>
                                                        <div class="card-body">
                                                            @foreach ($questions->where('group_id', $group->id) as $question)
                                                                <div class="mb-4 question" id="q{{ $question->id }}">
                                                                    <p><strong>Câu hỏi #{{ $question->id }}:</strong>
                                                                        
                                                                        {{ $question->content }}</p>
                                                                    @foreach ($answers->where('question_id', $question->id) as $answer)
                                                                        <div
                                                                            class="form-check{{ $answer->is_correct ? ' correct' : ' incorrect' }}">
                                                                            <input class="form-check-input" type="radio"
                                                                                name="q{{ $question->id }}"
                                                                                id="q{{ $question->id }}a{{ $loop->iteration }}"
                                                                                value="{{ $answer->id }}">
                                                                            <label class="form-check-label"
                                                                                for="q{{ $question->id }}a{{ $loop->iteration }}">{{ $loop->iteration }}.
